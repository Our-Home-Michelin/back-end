...
export const getRecipe = async (req, res) => {
    try {
        ...
        const recipe = await Recipe.findById(recipeId).populate("writer", "nickName");
        if (!recipe) {
            // 해당 레시피가 없는 경우
...
export const writeRecipe = async (req, res) => {
    try {
        const { title, recipeType, recipeServing, process, ingredients, imageUrl, createdDate, writer } = req.body;
...
export const uploadRecipeImage = async (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({ message: "레시피 대표 이미지 파일을 선택해주세요." });
        }
        ...
        res.status(500).json({ message: "레시피 대표 이미지 업로드 과정에서 문제가 발생했습니다." });
        console.log(err);
    }
};
